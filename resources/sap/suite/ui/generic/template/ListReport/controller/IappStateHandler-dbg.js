sap.ui.define(["jquery.sap.global", "sap/ui/base/Object",
	...
	function getMethods(oState, oController, oTemplateUtils) {
		...
		function getCurrentAppState() {
			 ...
			 */
			var oCurrentSelectionVariant = oState.oSmartFilterbar.getUiState().getSelectionVariant();
			var aLanguage = sap.ui.getCore().getConfiguration().getLanguage().split("-", 1);
			aLanguage[0] = aLanguage[0].toLowerCase();
			var iCount = 0;
			if (oState.oSmartFilterbar.getUiState().getValueTexts() && oState.oSmartFilterbar.getUiState().getValueTexts().Texts && oState.oSmartFilterbar.getUiState().getValueTexts().Texts.length > 1) {
				for (var i = 0; i < oState.oSmartFilterbar.getUiState().getValueTexts().Texts.length; i++) {
					if (oState.oSmartFilterbar.getUiState().getValueTexts().Texts[i].Language === aLanguage[0]) {
						iCount = i;
						break;
					}
				}
			}
			// Replacing PropertyValue with Text information if available
			if (oState.oSmartFilterbar.getUiState().getValueTexts() && oState.oSmartFilterbar.getUiState().getValueTexts().Texts) {
				for (var j = 0; j < oState.oSmartFilterbar.getUiState().getValueTexts().Texts[iCount].PropertyTexts.length; j++) {
					var aValueTexts = oState.oSmartFilterbar.getUiState().getValueTexts().Texts[iCount].PropertyTexts[j].ValueTexts;
					for (var i = 0; i < aValueTexts.length; i++) {
						if (aValueTexts[i]) {
							oCurrentSelectionVariant.SelectOptions[j].Ranges[i].Low = aValueTexts[i].Text;
						}
					}
				}
			}
			var sCurrentSelectionVariant = JSON.stringify(oCurrentSelectionVariant);
