...
sap.ui.define([
	 ...
	 */
	var fnGetUI5VersionInfo = function () {
	 ...
	 */
	var fnGetApplicationComponentByManifest = function (oManifest) {
	 ...
	 */
	var fnGetApplicationIDByManifest = function (oManifest) {
	 ...
	 */
	var fnGetRegistrationIDsByManifest = function (oManifest) {
	...
	function getIdFromView($View, sPattern) {
		for (var i = 0; i < $View.length; i++) {
			if ($View[i] && $View[i].id && $View[i].id.includes(sPattern)) {
	 ...
	 */
	var fnGetManifestByStructure = function () {
		...
		if ($OVPWrapper && $OVPWrapper.length && $OVPWrapper.length > 0) {
			// OVP
			$XMLView = $OVPWrapper.closest(".sapUiXMLView");
			if ($XMLView
				&& $XMLView.length
				&& $XMLView.length > 0
				&& $XMLView[0]
			...
			for (var i = 0; i < $XMLView.length; i++) {
				if ($XMLView[i].id.includes(sPattern)) {
	 ...
	 */
	var fnGetComponentIDByStructure = function () {
	 ...
	 */
	var fnGetManifestPath = function (sAppId) {
		if (!sAppId) {
			return "";
		}
	 ...
	 */
	var fnGetManifestURL = function (sOrigin, sPathname, sManifestPath) {
		...
		if (sManifestPath.startsWith("./") || sManifestPath.startsWith("../")) {
	 ...
	 */
	var fnGetRootPath = function (sManifestPath) {
		if (sManifestPath) {
	 ...
	 */
	var fnIsValidFloorplan = function (sFloorplan) {
		if (!sFloorplan) {
			return false;
		}
	 ...
	 */
	var fnGetTicketComponentForFloorplan = function (sFloorplan) {
	 ...
	 */
	var fnIsOverviewPageByManifest = function (oManifest) {
	 ...
	 */
	var fnIsListReportByManifest = function (oManifest) {
		if (!(oManifest
	 ...
	 */
	var fnIsAnalyticalListPageByManifest = function (oManifest) {
		if (!(oManifest
			&& oManifest["sap.ui.generic.app"]
			&& oManifest["sap.ui.generic.app"].pages
			&& oManifest["sap.ui.generic.app"].pages !== [])) {
	 ...
	 */
	var fnGetFloorplanByManifest = function (oManifest) {
		if (oManifest) {
	 ...
	 * @returns {boolean} true if OverviewPage, otherwise false
	 */
	var fnIsOverviewPageByStructure = function () {
	 ...
	 * @returns {boolean} true if ListReport, otherwise false
	 */
	var fnIsListReportByStructure = function () {
	 ...
	 * @returns {boolean} true if AnalyticalListPage, otherwise false
	 */
	var fnIsAnalyticalListPageByStructure = function () {
	 ...
	 * @returns {string} mApplicationType
	 */
	var fnGetFloorplanByStructure = function () {
		var oManifest = fnGetManifestByStructure();
