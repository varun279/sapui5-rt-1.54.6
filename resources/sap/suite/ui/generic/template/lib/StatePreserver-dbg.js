sap.ui.define(["jquery.sap.global", "sap/ui/base/Object", "sap/suite/ui/generic/template/lib/testableHelper"
	...
	*/
	function getMethods(oTemplateContract, oSettings) {
		...
		var sLastPath, sLastAppKey, sLastBindingPath;
		...
		function fnTransferAnalyzedStateToUrl(){
			...
			if (sAppStateKeyInUrl === oStoringInformation.appStateKey){ // if the appstateKey really represents a new state set it to hash
				...
				oTemplateContract.oNavigationControllerProxy.navigateByExchangingQueryParam(oSettings.appStateName, oStoringInformation.appStateKey, true);                        
			}			
		}
		...
		function getUrlParameterInfo(sPath){
			return oAppStateIsSetPromise.then(function(){
				...
				var sKey = (sLastPath === sPath) && sLastAppKey;
				if (sKey){
		...
		function fnAdaptRealizedAppStateKey(sAppStateKey){
			...
			var sPath = oTemplateContract.oNavigationControllerProxy.getCurrentHash(oRegistryEntry.viewLevel);
			sLastPath = sPath;
		...
		function isStateChange(oEvent){
			...
			sAppStateKeyInUrl = (oQuery && oQuery[oSettings.appStateName]) || "";
			if (oStoringInformation){
				if (oStoringInformation.appStateKey !== sAppStateKeyInUrl){
					jQuery.sap.log.error("StatePreserver: Got AppstateKey " + sAppStateKeyInUrl + " expected " + oStoringInformation.appStateKey);
					return false;
				}
