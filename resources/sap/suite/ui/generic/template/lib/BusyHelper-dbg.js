sap.ui.define(["jquery.sap.global", "sap/ui/base/Object", "sap/suite/ui/generic/template/lib/MessageUtils", "sap/suite/ui/generic/template/lib/testableHelper"],
	function(jQuery, BaseObject, MessageUtils, testableHelper) {
		...
		function getMethods(oTemplateContract) {
			...
			function logPromiseWithCallerFromStack(directCaller, busyEndedPromise) {
				var aStack, sCaller, iLevel;

				//throw an error to get the caller from the stack
				try {
					throw new Error("Get the stack");
				} catch (e) {
					aStack = e.stack.split(directCaller, 2);
					if (aStack.length >= 2) {
				...
				//make sure the info message with support info is logged also for other log levels
				iLevel = jQuery.sap.log.getLevel();
				if (iLevel < jQuery.sap.log.Level.INFO) {
					jQuery.sap.log.setLevel(jQuery.sap.log.Level.INFO);
				}
				jQuery.sap.log.info("busyHandling: " + directCaller + " called " + sCaller, function() {
					var oSupportInfo = {
						type: "sap.suite.generic.template.busyHandling",
			...
			function setBusy(oBusyEndedPromise, bImmediate, oSessionParams) {
				...
				if (sap.ui.support) { //only if support assistant is loaded
					logPromiseWithCallerFromStack("setBusy", oBusyEndedPromise);
				}
				fnMakeBusy();
