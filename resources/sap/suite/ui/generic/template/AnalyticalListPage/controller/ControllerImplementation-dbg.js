			...
			getMethods: function(oViewProxy, oTemplateUtils, oController) {
				...
				function onSmartFilterBarInitialise(oEvent){
					...
					if (oDefaultFilterSuiteFormat) {
						oState.oIappStateHandler.fnSetFiltersUsingUIState(JSON.stringify(oDefaultFilterSuiteFormat), true, false);
					}
				...
				function onSmartFilterBarInitialized(){
					...
					oAppStatePromise.then(function(){
						...
						oState.oIappStateHandler.fnSetDefaultFilter();
						oState.oIappStateHandler.fnUpdateSVFB();
						bIsStartingUp = false;
					});
				}
							...
							oContent.forEach(function(oItem) {
								if (oItem.getSmartFilterId) {
									oItem.attachBeforeRebindFilterableKPI(function(oEvent) {
										...
										oExtController.onBeforeRebindFilterableKPIExtension(oSelectionVariant, sEntityType);
									}, oExtController);
								}
							});
						}
					...
					handlers: {
						...
						onSearchButtonPressed: function() {
							...
							var fnRequestFailed = function(oEvent) {
								...
								var modelUpdatedPromise = oController && oController.customizeMsgModelforTransientMessages();
								if (modelUpdatedPromise){
									modelUpdatedPromise.then(function(){
