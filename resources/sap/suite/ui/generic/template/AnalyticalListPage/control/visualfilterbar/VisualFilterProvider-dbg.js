...
sap.ui.define([
		...
		if (valueListProperties !== undefined) {
			...
			if (presentationVariantQualifierString) {
				...
				if (dimension) {
					...
					// Needs to be done
					annotationObject = this._createConsumeableObjectFromAnnotation(presentationVariantAnnotation, collectionPath, isSelected, parameters, parentProperty, filterRestriction, isDropDown, bIsMandatoryProp, displayBehaviour, entityTypeCollectionPath);
					if (annotationObject) {
	...
	*/
	...
	/*
		...
		and form the config.
	*/
	VisualFilterProvider.prototype._createConsumeableObjectFromAnnotation = function (presentationVariantAnnotation, collectionPath, isSelected, parameters, parentProperty, filterRestriction, isDropDown, bIsMandatoryProp, displayBehaviour, entityType) {
		...
		if (!bIsValid) {
			jQuery.sap.log.error("The dimension " + dimension + " or the measure " + measure + " or both are either NOT given in the chart, or IS given but undefined in metadata, or have NO 'sap:aggregation-role' defined in metadata. Corresponding chart cannot be displayed as VisualFilter.");
			return bIsValid;
		}
	...
	*/
	VisualFilterProvider.prototype._isValidDimensionandMeasure = function(sEntitySet, sDimension, sMeasure) {
		...
		var oMeasureMap = this.getMeasureMap(),
			oMeasureFieldInfo = FilterUtil.readProperty(oMeasureMap, sEntitySet + "." + sMeasure + ".fieldInfo");

		if (!(oDimensionFieldInfo && oMeasureFieldInfo)) {
			return false;
		}

		if (!(oDimensionFieldInfo['aggregationRole'] === "dimension" &&
			oMeasureFieldInfo['aggregationRole'] === "measure")) {
			return false;
		}
