...
sap.ui.define([
	...
	function(library, coreLibrary, jQuery, HtmlElement, Renderer) {
		"use strict";
		...
		function getClassBySize(oStatusIndicator, sPostfix) {
			...
			switch (oStatusIndicator.getSize()) {
				...
				case oSizeType.None:
					...
					break;
				default:
					jQuery.sap.log.error("Unknown size. Expecting size defined in sap.suite.ui.commons.statusindicator.SizeType," +
		...
		StatusIndicatorRenderer._getHtmlModel = function(oStatusIndicator) {
			...
			oRootElement.setAttribute("aria-label", sAriaLabel ? sAriaLabel : resourceBundle.getText("STATUS_INDICATOR_ARIA_LABEL"));

			var aAriaLabelledBy = oStatusIndicator.getAriaLabelledBy();

			if (aAriaLabelledBy && aAriaLabelledBy.length > 0) {
				oRootElement.setAttribute("aria-labelledby", aAriaLabelledBy.join(" "));
			}

			var aAriaDescribedBy = oStatusIndicator.getAriaDescribedBy();

			if (aAriaDescribedBy && aAriaDescribedBy.length > 0) {
				oRootElement.setAttribute("aria-describedby", aAriaDescribedBy.join(" "));
			}
