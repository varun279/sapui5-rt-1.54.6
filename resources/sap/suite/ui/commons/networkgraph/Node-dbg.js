sap.ui.define([
	...
	Node.prototype._renderContent = function (mOptions) {
		var sNodeHtml = this._isBox() ? this._renderBox(mOptions) : this._renderCircle();
	...
	Node.prototype._renderCircle = function () {
		...
		sHtml += this._renderInfoIcon(bIsRtl ? this._getCirclePosition().x + iCircleSize - iInfoOffsetX : this._getCirclePosition().x + iInfoOffsetX, fY + iInfoOffsetY);
	...
	Node.prototype._renderBox = function (mOptions) {
		...
		if (this._displayDescription()) {
			sHtml += this._renderDescription({
				...
				y: fY + this._getTitleHeight() + (this._hasMultiLineTitle() ? (Size.Title.OFFSET * 2 + Size.Title.OFFSET / 2) : Size.Title.OFFSET * 3)
			});
		}
	...
	Node.prototype._getNodeClass = function () {
		...
		return "sapSuiteUiCommonsNetworkNode " + sSelectedClass + sCustomStatusClass +
	...
	Node.prototype._createDescription = function () {
		...
		if (this._displayDescription() && $node[0]) {
			...
			// we have to set font size by style because classes may not be downloaded yet
			oText.setAttribute("style", "font-size:0.75rem");
	...
	Node.prototype._renderMultilineText = function (mAttributes) {
		...
		for (var i = 0; i < mAttributes.lines.length; i++) {
			sHtml += this._renderText({
				attributes: {
					style: this._isBox() ? this._getColorStyle({
						fill: ElementBase.ColorType.Content
					}) : "",
					"class": mAttributes.class + (this._isBox() ? " sapSuiteUiCommonsNetworkStatusText " : ""),
					x: mAttributes.x,
	...
	Node.prototype._setStatusColors = function (sType) {
		if (this._hasCustomStatus()) {
			var $box = this.$("innerBox"),
				sContentColor = this._getColor(ElementBase.ColorType[sType + "Content"]),
				...
				$infoIconWrapper = this.$().find(".sapSuiteUiCommonsNetworkNodeInfoBox");

			$box.css("fill", sBackgroundColor);
			$box.css("stroke", sBorderColor);
			this.$().find(".sapSuiteUiCommonsNetworkStatusText").css("fill", sContentColor);

			// info icon
			$infoIconWrapper.css("fill", sBackgroundColor);
			$infoIconWrapper.css("stroke", sBorderColor);
			this.$().find(".sapSuiteUiCommonsNetworkNodeInfoIcon").css("fill", sContentColor);
	...
	Node.prototype._isIgnored = function () {
		return (this.getParent()._getLayoutAlgorithm().isLayered() && this._oGroup
			&& (this._oGroup.getCollapsed() || this._oGroup._isIgnored()))
			|| !this._useInLayout();
	};
