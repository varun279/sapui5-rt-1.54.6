sap.ui.define([
	...
	"sap/ui/core/theming/Parameters"
			 ...
			 ToolbarSpacer, CustomData, Filter, OverflowToolbarLayoutData, KeyboardNavigator, Parameters) {
	...
	Graph.prototype.onAfterRendering = function () {
		...
		if (this.getNodes().length === 0 || this._bIsInvalid) {
			if (this._bIsInvalid) {
	...
	Graph.prototype._fireFailure = function (oType, sMsg) {
		...
		this.fireFailure([oType.toString(), sParsedMsg]);
		jQuery.sap.log.error("Graph failure: " + oType + ": " + sParsedMsg);
	};
	...
	Graph.prototype._innerRender = function (mParams) {
		...
		var SIZE_OFFSET_Y = 20,
			...
			fnSetLimits = function () {
				...
				this._iWidth = Math.round(iMaxX + SIZE_OFFSET_X);
	...
	Graph.prototype._renderSvg = function (sNodes, sLines, sGroups) {
		...
		sHtml += "<g id=\"" + this.getId() + "-eventwrapper\"><rect style=\"fill: none\" class=\"sapSuiteUiCommonsNetworkGraphEventWrapper\" width=\"100%\" height=\"100%\"/></g>";
	...
	Graph.prototype._setupKeyboardNavigation = function () {
		...
		this.getLines().forEach(function (oLine) {
			if (oLine.isHidden() || oLine._isIgnored() || !oLine.getVisible()) {
				return;
			}
	...
	Graph.prototype._preventZoomToChangeLineWidth = function () {
		return this._isLayered() && (this._fZoomRatio >= 0.5 && this._fZoomRatio < 2);
	};

	Graph.prototype._setLineClasses = function () {
		var $this = this.$(),
			sFnName = this._preventZoomToChangeLineWidth() ? "addClass" : "removeClass",
			sFnCrispName = (this._fZoomRatio < 0.5) ? "removeClass" : "addClass";
	...
	Graph.prototype._zoom = function (mParameters) {
		...
		this.$svg.width(Math.round(this._iWidth * this._fZoomRatio));
	...
	Graph.prototype._ensureOnScreen = function (oItem) {
		var oScroller, oArrowPos, iLeft, iRight, iTop, iBottom, iCenterX, iCenterY;
		...
		if (oItem instanceof Node) {
			...
			oArrowPos = oItem._getArrowPosition();
			iCenterX = (oArrowPos.arrowFrom.x + oArrowPos.arrowTo.x) / 2;
			iCenterY = (oArrowPos.arrowFrom.y + oArrowPos.arrowTo.y) / 2;
		} else if (oItem instanceof Group) {
			iCenterX = oItem.getX();
			iCenterY = oItem.getY();
		}
		iCenterX *= this._fZoomRatio;
		iCenterY *= this._fZoomRatio;

		iLeft = oScroller.scrollLeft;
		iTop = oScroller.scrollTop;
		iRight = iLeft + this.$scroller.width();
		iBottom = iTop + this.$scroller.height();
		if (iCenterX < iLeft || iCenterX > iRight) {
			oScroller.scrollLeft = iCenterX - this.$scroller.width() / 2;
		}
		if (iCenterY < iTop || iCenterY > iBottom) {
			oScroller.scrollTop = iCenterY - this.$scroller.height() / 2;
		}
	};
	...
	Graph.prototype._createLegend = function () {
		...
		var fnAppendLine = function (sStatus, sType) {
			var oCustomStatus = this._oStatuses[sStatus],
				...
				sCustomTitle = oCustomStatus && oCustomStatus.getTitle(),
				sStyle = sColor ? "background-color:" + Parameters.get(sColor) : "";

			sCustomTitle = sCustomTitle || sStatus;
			...
			sHtml += this._oLegendLabels[sType + sStatus] ? this._oLegendLabels[sType + sStatus] : sCustomTitle;
			sHtml += "</label>";
			sHtml += "</div>";
		}.bind(this);
		...
		var fnProcessElements = function (aItems, oCollection, sClass, sTitle, sStatusType) {
			...
			if (fnHasStatus(oCollection)) {
				sHtml += "<div class=\"" + sClass + "\"><label class=\"sapSuiteUiCommonsNetworkGraphLegendTitle\">" + jQuery.sap.encodeHTML(oResourceBundle.getText(sTitle)) + "</label></div>";
				Object.keys(oCollection).forEach(function (sKey) {
					fnAppendLine(sKey, sStatusType);
				});
			}
		};
	...
	Graph.prototype._isDelayedLayouting = function () {
		return this.getNodes().some(function (oNode) {
			return oNode._useAutomaticSize() || oNode.getTitleLineSize() !== 1 || oNode._displayDescription();
		});
	};
