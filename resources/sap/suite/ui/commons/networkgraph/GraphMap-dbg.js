sap.ui.define([
	...
	GraphMap.prototype._renderMap = function () {
		...
		if (!oGraph._iWidth || !oGraph._iHeight || iNodeCount === 0) {
			if (oGraph._bIsInvalid) {
				this.setBusy(false);
			}
			return;
		}
	...
	GraphMap.prototype._correctLineWidth = function (oGraph) {
		var oGraph = this.getGraph(),
			bCorrectWidth = this._isMSBrowser() ||
				(oGraph && !oGraph._preventZoomToChangeLineWidth() && oGraph.getNodes().length < this.getDirectRenderNodeLimit());
