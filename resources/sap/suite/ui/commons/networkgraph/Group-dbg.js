sap.ui.define([
	...
	var Group = ElementBase.extend("sap.suite.ui.commons.networkgraph.Group", {
		metadata: {
			...
			events: {
				...
				/**
				 * This event is fired when the user clicks or taps the header of the group.
				 */
				press: {}
			}
		}
	...
	Group.prototype._render = function (mOptions) {
		...
		if (this._isIgnored() || this._isEmpty() || !this.getVisible()) {
			return "";
		}
		...
		if (this.getParent()._isLayered()) {
			...
			if (this.getCollapsed()) {
				...
				sGroupHtml += this._renderRoundRect({
					...
					height: HEADER_SIZE,
					"class": "sapSuiteUiCommonsNetworkGroupHeader",
					topRight: Size.ROUNDING,
					topLeft: Size.ROUNDING
				});
			}

			// focus
			sGroupHtml += this._renderControl("rect", {
				id: this.getId() + "-focusheader",
				x: fX + FOCUS_OFFSET,
				style: this._getColorStyle({
	...
	Group.prototype._renderHeaderButtons = function () {
		...
		fnCreateIcon(oResourceBundle.getText("NETWORK_GRAPH_EXPAND_COLLAPSE"), this.getCollapsed() ? sIconExpand : sIconCollapse, "collapse", Size.ROUNDING, this.getCollapsed() ? Size.ROUNDING : 0);
	...
	Group.prototype._setupEvents = function () {
		var $menu = this.$("menu"),
			...
			$actionButtons = this.$().find(".sapSuiteUiCommonsNetworkGroupHeaderActionIconWrapper");
		...
		$actionButtons.mouseover(function (oEvent) {
			var $icon = jQuery(oEvent.currentTarget);

			$icon.css("fill", this._getColor(ElementBase.ColorType.HoverBackground));
			$icon.parent().find("text").css("fill", this._getColor(ElementBase.ColorType.HoverContent));
			$icon.css("stroke", this._getColor(ElementBase.ColorType.HoverBorder));
		}.bind(this));
		...
		$actionButtons.mouseout(function (oEvent) {
			...
			$icon.css("fill", "");
			$icon.parent().find("text").css("fill", this._getColor(ElementBase.ColorType.Content));
			$icon.css("stroke", "");
		}.bind(this));
	};
