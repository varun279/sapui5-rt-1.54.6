			...
			Timeline.prototype._setupRangeFilterPage = function () {
				...
				this._objects.register("timeRangeSlider", function () {
					...
					oSlider._updateTooltipContent = function (oTooltip, iNewValue) {
						var oDate, oDateLabel;

						var fnIsYear = function () {
							return that._rangeFilterType === TimelineGroupType.Year;
						};

						if (oDate > that._maxDate) {
							oDate = that._maxDate;
						}
						if (oDate < that._minDate) {
							oDate = that._minDate;
						}

						try {
							oDate = that._fnAddDate(iNewValue);
							oDateLabel = that._formatGroupBy(oDate, that._rangeFilterType).title;
							// we have to set value to correct tooltip position
							// we will replace tooltip text later on
							oTooltip.setValue(iNewValue);
							oTooltip.onAfterRendering = function () {
								try {
									if (this instanceof SliderTooltip && SliderTooltip.prototype.onAfterRendering) {
										SliderTooltip.prototype.onAfterRendering.apply(this);
									}

									var $input = oTooltip.$("input");
									$input.attr("type", null);
									$input.val(oDateLabel);
									$input.parent().width(fnIsYear() ? "40px" : "115px");
								} catch (e) {
									// nothing
								}
							};

							oTooltip.$("input").attr("type", null);
							oTooltip.$("input").val(oDateLabel);
						} catch (e) {
							// no need to react, just don't show labels
						}
					};
