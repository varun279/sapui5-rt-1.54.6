...
sap.uiext.inbox.composite.InboxTaskComments.prototype._sanitizeText=function(t){var s=jQuery.sap._sanitizeHTML(t);return s.replace(/&amp;/g,'&').replace(/&lt;/g,'<').replace(/&gt;/g,'>').replace(/&quot;/g,'"');};
sap.uiext.inbox.composite.InboxTaskComments.prototype.showBusy=function(s){if(s){var b=new sap.uiext.inbox.composite.InboxBusyIndicator();b.setBusy(true);this.setAggregation('busyIndicator',b);}else{this.destroyAggregation('busyIndicator');}};
