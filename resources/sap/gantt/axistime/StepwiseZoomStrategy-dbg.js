sap.ui.define(["./AxisTimeStrategyBase", "sap/gantt/misc/Format"], function (AxisTimeStrategyBase, Format) {
	...
	StepwiseZoomStrategy.prototype.init = function() {
		...
		this._initialTotalHorizon = null;
	...
	StepwiseZoomStrategy.prototype._getIndexOfTimeLineOption = function (oTimeLineOption, oTimeLineOptions) {
		var iIndex, oOptions = oTimeLineOptions;
		if (!oOptions) {
	...
	StepwiseZoomStrategy.prototype._extendTotalHorizon = function(nClientWidth){
		if (!this._initialTotalHorizon) {
			this._initialTotalHorizon = this.getTotalHorizon();
		}
		var oTotalHorizon = this.getTotalHorizon();
	...
	StepwiseZoomStrategy.prototype._calTotalHorizon = function(nTimeSpanDelta, nNewTimeSpan) {
		var oTotalHorizon = this.getTotalHorizon();
		if (nTimeSpanDelta !== 0) {
			var nInitialTotalHorizonStartTimeInMs = Format.abapTimestampToDate(this._initialTotalHorizon.getStartTime()).getTime();
			var nInitialTotalHorizonEndTimeInMs = Format.abapTimestampToDate(this._initialTotalHorizon.getEndTime()).getTime();
			...
			if ( nTimeSpanDelta > 0 ){
				...
				var nNewTotalHorizonStartTimeInMs = Format.abapTimestampToDate(oTotalHorizon.getStartTime()).getTime() - nTimeSpanDelta/2;
				var nNewTotalHorizonEndTimeInMs = Format.abapTimestampToDate(oTotalHorizon.getEndTime()).getTime() + nTimeSpanDelta/2;
