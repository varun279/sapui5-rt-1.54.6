...
sap.ui.define([
	...
	"sap/gantt/misc/ShapeManager", "sap/gantt/shape/Group", "sap/gantt/eventHandler/ShapeResizeHandler", "sap/gantt/eventHandler/ShapeHoverHandler","sap/gantt/eventHandler/TimePopoverHandler", "sap/gantt/eventHandler/BirdEyeHandler",
	// 3rd party lib
	"sap/ui/thirdparty/d3"
		...
		ShapeManager, Group, ShapeResizeHandler, ShapeHoverHandler, TimePopoverHandler, BirdEyeHandler) {
	"use strict";
	...
	GanttChart.prototype._handleShapeDragStart = function (oEvent) {
		...
		if (oSourceShapeData && bEnableDnD) {
			...
			if (rowInfo) {
				...
				var oDragStartPoint = {"x": x, "y": y, "shapeX": shapeX, "shapeWidth": shapeWidth, "shapeY": shapeY};
	...
	GanttChart.prototype._resetShadowRelativePosition =  function (oClonedDom, relativeX, relativeY) {
		...
		if (aChildNodes && aChildNodes.length > 0) {
			if (!relativeX) {
				var iNodeX = jQuery(aChildNodes[0]).attr("x");
				var iNodeY = jQuery(aChildNodes[0]).attr("y") - 2;
	...
	GanttChart.prototype._handleSelectionChange = function (oEvent) {
		...
		if (oShapeData !== undefined && this.isShapeSelectable(oShapeData, sClassId)) {
			...
			var oGanttChartWithTable = this.getParent();
			if (oGanttChartWithTable.getTableProperties().selectionBehavior !== sap.ui.table.SelectionBehavior.RowSelector) {
				var param = this._composeParameterForClickPosition(oEvent);
	 ...
	 * @private
	 */
	GanttChart.prototype.getLargestHorizonByDataRange = function (sBirdEyeRange, iRowIndex) {
		...
		return this._oBirdEyeHandler.calculateLargestVisibleHorizon(aAllShapeInstances);
	};
