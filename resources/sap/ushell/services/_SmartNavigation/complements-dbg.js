...
(function (sap) {
    ...
    /**
     * Determines the frequency of navigation between the given origin hash and
     ...
     *
     * @returns {Array} List of navigation occurrences originating from the given
     * hash. Each item contains a destination hash and the frequency of the
     * occurence of navigations between the origin and the destination.
     *
     * @private
     */
    ...
    /**
     * The function completely delegates to `mapClickCountsIntoLinkItems`. The
     * essence of its existence is to support fluency in source code readability
     * and thus to psychologically enhance ease of maintenance.
     *
     * This function ultimately mutates individual items in the aLinks list,
     * because `mapClickCountsIntoLinkItems` does mutate the `aLinks` that is
     * passed to it.
     *
     * @param {Array} aLinks List of link items for which a click count should be inserted.
     * @param {Array} aNavigationOccurrences List of known navigation occurrences
     * from which the click count should be deduced.
     * @param {object} oURLParsing The URL parsing service.
     *
    ...
    /**
     * Record the occurrences of navigation from `sFromCurrentShellHash` to
     * `sToDestinationShellHash` and persists it to a remote storage accessed with
     * `oPersonalizationStore`, in the context of the currently running application.
     *
     ...
     * @param {object} oPersonalizationStore An interface to access the remote
     * persistent store where the record should be stored.
    ...
    /**
     * Updates the given history item which may be either an instance derived
     * from either factories `SemanticObjectHistoryEntry` or `SemanticObjectHistoryEntry`.
     *
     * Note that the update is done in place. In other words, the passed
     * `oHistoryEntry` reference will be mutated.
     *
     ...
     *
     * A SemanticObjectHistoryEntry records the frequency of navigation from a
     * given semantic object to various actions, held in its `actions`
     * collection.
     *
     * It also holds information about the latest time of visiting a
     * particular action and the daily frequency of visits in previous days.
     *
     * The dailyFrequency property list is structured in such a way that the
     * number at index 0 counts the number of visits recorded for the latest
     * day of visit, and at index 1 exists the number of visits one day earlier,
     ...
     *
     * Irrespective of an associated semantic object, an instance of an
     * ActionHistoryEntry holds information about the latest time of visiting a
     * particular action and the daily frequency of visits in previous days.
     *
     * The dailyFrequency property list is structured in such a way that the
     * number at index 0 counts the number of visits recorded for the latest
