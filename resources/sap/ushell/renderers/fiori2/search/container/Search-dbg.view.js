    ...
    return sap.ui.jsview("sap.ushell.renderers.fiori2.search.container.Search", {
        ...
        createContent: function(oController) {
            ...
            var didYouMeanBar = new sap.m.MessageStrip({
                text: sap.ushell.resources.i18n.getText('did_you_mean', '{/uiFilter/searchTerm}'),
                showIcon: true,
        ...
        assembleSearchToolbar: function(bWithoutShareButton) {
            ...
            var dataExportButton = new sap.m.Button("dataExportButton", {
                ...
                //                },
                //                press: function() {
                //                    that.searchResultTable.onDataExport();
                //                }
            });
            //dataExportButton.addStyleClass("sapUshellSearchdataExportButton");
        ...
        updatePersoServiceAndController: function() {
            ...
            //TODO: relocate updateTableColumnPersoOrder 
            //that.updateTableColumnPersoOrder();
        },

        updateTableColumnPersoOrder: function() {
            var that = this;
            var model = that.getModel();
            var dsKey = model.getDataSource().id;
            var serviceId = model.getPersonalizationStorageInstance().getPersonalizer('search-result-table-state-' + dsKey);

            that.oTablePersoController.getPersoService(serviceId).getPersData().then(function(data) {
                if (data) {
                    var persoColumns = data.aColumns;
                    var modelColumns = this.getModel().getProperty("/tableColumns");
                    modelColumns.forEach(function(modelColumn) {
                        persoColumns.forEach(function(persoColumn) {
                            var key = persoColumn.id.substring(persoColumn.id.lastIndexOf("-") + 1);
                            if (modelColumn.key === key) {
                                modelColumn.persoOrder = persoColumn.order;
                            }
                        });
                    });
                }
