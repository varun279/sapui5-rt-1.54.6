...
sap.ui.define([
	...
	SceneOrientationToolHandler.prototype._insideButton = function(event) {
		var gizmo = this._tool.getGizmo();
		var domobj = gizmo ? gizmo._button.getDomRef() : null;
		if (!domobj) {
			return false;
		}

		var offset = this._getOffset(domobj);
		return event.x >= offset.x && event.x <= offset.x + domobj.offsetWidth &&
			event.y >= offset.y && event.y <= offset.y + domobj.offsetHeight;
	};
