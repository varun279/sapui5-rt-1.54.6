...
sap.ui.define([
		...
		if (resources && resources.Set && resources.Set.Resource) {
			...
			var processResource = function(r) {
				if (jQuery.sap.endsWith(r.name, ".dae")) {
					this._context.resources.set(r.name, resourceToFile(r.value, r.name));
				} else if (jQuery.sap.endsWith(r.name, ".png") || jQuery.sap.endsWith(r.name, ".jpg")) {
					this._context.resources.set(r.name, "data:image/" + r.name.split(".")[1] + ";base64," + r.value);
				} else {
					this._context.resources.set(r.name, "data:" + r.name.split(".")[1] + ";base64," + r.value);
				}
			};
			toArray(resources.Set.Resource).forEach(processResource, this);
		}
	...
	VBIJSONParser.prototype._processScenes = function(data) {
		...
		if (data.Set) {
			...
			setElements.forEach(function(setElement) {
				toArray(setElement.Scene).forEach(function(sceneElement) {
					var scene = this._factory.createScene();
		...
		if (data.Merge) {
			...
			if (mergeElements.length > 0) {
				...
				mergeElements.forEach(function(mergeElement) {
					toArray(mergeElement.Scene).forEach(function(sceneElement) {
