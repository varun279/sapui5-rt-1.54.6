...
sap.ui.define([
	"jquery.sap.global", "sap/ui/base/Object", "./Utilities", "./thirdparty/three", "./thirdparty/ColladaLoader"
	...
	var SceneBuilder = BaseObject.extend("sap.ui.vbm.adapter3d.SceneBuilder", /** @lends sap.ui.vbm.adapter3d.SceneBuilder.prototype */ {
		...
		constructor: function(context, root, controller) {
			BaseObject.call(this);

			this._context = context;

			// Camera Controller from Viewport
	...
	SceneBuilder.prototype.synchronize = function() {
		...
		return new Promise(function(resolve, reject) {
			...
			// Setup initial view, flag set to true if most recent payload has 3d scene with camera setup instructions
			if (that._context.setupView) {
				that._setupView(scene.position, scene.zoom, scene.pitch, scene.yaw);
				that._context.setupView = false; //reset flag
			}
	...
	SceneBuilder.prototype._setupView = function(position, zoom, pitch, yaw) {
		...
		this._controller.reset({
			...
			zoom: this._controller.saveState().zoom,
	...
	SceneBuilder.prototype._loadTexture = function(textureResourceName) {
		var that = this;
		// TODO: test that the texture resource exists in this._context.resources.
		return new Promise(function(resolve, reject) {
			that._getTextureLoader().load(
				that._context.resources.get(textureResourceName),
				function(texture) {
		...
		return new Promise(function(resolve, reject) {
			try {
				that._getColladaLoader().parse(
					that._context.resources.get(modelResourceName),
					function(collada) {
