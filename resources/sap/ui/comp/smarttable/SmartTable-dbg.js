	...
	SmartTable.prototype._triggerUI5ClientExport = function() {
		...
		oExportLibLoadPromise.then(function() {
			...
			if (!this._InterceptService) {
				var _InterceptService = sap.ui.require("sap/ushell/cloudServices/interceptor/InterceptService");
				if (_InterceptService) {
					this._InterceptService = _InterceptService.getInstance();
				}
			}
			// Use Url from the intercept service, if interceptUrl API exists!
			if (this._InterceptService) {
				sDataUrl = this._InterceptService.interceptUrl ? this._InterceptService.interceptUrl(sDataUrl) : sDataUrl;
				sServiceUrl = this._InterceptService.interceptUrl ? this._InterceptService.interceptUrl(sServiceUrl) : sServiceUrl;
			}
			var mExportSettings = {
				workbook: {
