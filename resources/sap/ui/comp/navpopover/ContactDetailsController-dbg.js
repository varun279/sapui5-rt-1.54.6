...
sap.ui.define([
	'jquery.sap.global', 'sap/ui/comp/library', 'sap/ui/core/Element', 'sap/ui/layout/form/SimpleForm', 'sap/ui/layout/form/SimpleFormLayout', 'sap/m/Title', 'sap/ui/core/TitleLevel', 'sap/m/Label', 'sap/m/Text', 'sap/m/Image', 'sap/m/Link', 'sap/ui/base/ManagedObjectObserver', 'sap/ui/comp/odata/MetadataAnalyser'
], function(jQuery, CompLibrary, Element, SimpleForm, SimpleFormLayout, Title, TitleLevel, Label, Text, Image, Link, ManagedObjectObserver, MetadataAnalyser) {
	"use strict";
	...
	ContactDetailsController.prototype.getBindingPath4ContactAnnotation = function(sBindingPath, sContactAnnotationPath, sKey) {
		...
		return oMetaModel.loaded().then(function() {
			...
			try {
				...
				jQuery.sap.log.error("sap.ui.comp.navpopover.ContactDetailsController.getBindingPath4ContactAnnotation: binding path '" + sBindingPath + "' is not valid. Error has been caught: " + oError);
				return null;
			}
			...
			try {
				...
				jQuery.sap.log.error("sap.ui.comp.navpopover.ContactDetailsController.getBindingPath4ContactAnnotation: the path of sContactAnnotationPath '" + sBindingPath + "/" + sContactAnnotationPath + "' is not valid. Error has been caught: " + oError);
				return null;
			}
		});
	};
	...
	ContactDetailsController.prototype.isPropertyHidden = function(sPropertyPath) {
		var oMetaContext;
		if (!this.getModel()) {
			jQuery.sap.log.error("sap.ui.comp.navpopover.ContactDetailsController.isPropertyHidden: the model does not exist");
			return false;
		}
		// ODataModel returns MetaModel, JSONModel returns undefined
		if (!this.getModel().getMetaModel()) {
			jQuery.sap.log.error("sap.ui.comp.navpopover.ContactDetailsController.isPropertyHidden: the model should be the ODataModel");
			return false;
		}
		...
		try {
			...
			jQuery.sap.log.error("sap.ui.comp.navpopover.ContactDetailsController.isPropertyHidden: the path of property path '" + sPropertyPath + "' is not valid. Error has been caught: " + oError);
			return false;
		}
