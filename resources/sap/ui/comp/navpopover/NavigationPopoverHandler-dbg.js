...
sap.ui.define([
...
], function(jQuery, CompLibrary, ManagedObject, SemanticObjectController, JSONModel, Control, Factory, NavigationPopover, Util, VBox, LinkData, MessageBox, Controller, PersonalizationUtil, FlexHandler, NavigationContainer, ContactDetailsController) {
	"use strict";
	...
	/**
	 * Opens the <code>sap.m.Popover</code> control with navigation targets in an asynchronous manner.
	 *
	 * @param {object} oDomRef Optional DOM reference to which the popover is attached. By default the
	 * <code>control</code> association is used as DOM reference.
	 *
	...
	NavigationPopoverHandler.prototype._initModel = function() {
		...
		if (!sBindingPath) {
			jQuery.sap.log.warning("sap.ui.comp.navpopover.NavigationPopoverHandler: Binding Context is null. Please be aware that without binding context no semantic attributes can be calculated. Without semantic attributes no URL parameters can be created.");
		}
		...
		return Util.retrieveSemanticObjectMapping(this.getFieldName(), oODataModel, sBindingPath).then(function(oSemanticObjects) {
			...
			that._setSemanticAttributes(that._calculateSemanticAttributes(oSemanticObjects));
			...
			return Util.retrieveNavigationTargets(sSemanticObjectDefault, aAdditionalSemanticObjects, oResultFromOpen.appStateKey, oComponent, oSemanticAttributes, sMainNavigationId);
			...
			return that._fireNavigationTargetsObtained(sMainNavigationId, sSemanticObjectDefault, oSemanticAttributes, sId, aForms, aNavigationTargets);
