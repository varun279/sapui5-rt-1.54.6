...
sap.ui.define([
	...
	SmartChart.prototype._updateDrillBreadcrumbs = function() {
		...
		if (aVisibleDimensionsRev) {
			...
			aVisibleDimensionsRev
					.forEach(function(dim, index, array) {

						// Check if stack entry has dimension names and if a
						// dimension is existing for this name
						if (dim.dimension.length > 0
								&& typeof this._oChart
										.getDimensionByName(dim.dimension[dim.dimension.length - 1]) != 'undefined') {
							// show breadcrumbs
							...
							// up the drill-path
							var sDimLabel = this._oChart.getDimensionByName(
									dim.dimension[dim.dimension.length - 1])
									.getLabel();

							// Set current drill position in breadcrumb control
							if (index == 0) {

								this._oDrillBreadcrumbs
										.setCurrentLocationText(sDimLabel);
							} else {

								var oCrumb = new sap.m.Link({
									text : sDimLabel,
									press : function(oEvent) {
										var iLinkIndex = this._oDrillBreadcrumbs
												.indexOfLink(oEvent.getSource());
	...
	SmartChart.prototype._personalisationModelDataChange = function(oEvent) {
		...
		if (sChangeStatus === sap.ui.comp.personalization.ChangeType.Unchanged) {
			return;
		}
		...
		if (sChangeStatus === sap.ui.comp.personalization.ChangeType.TableChanged) {
			if (this._oSemanticalNavButton) {
				this._oSemanticalNavButton.setEnabled(false);
			}
		} else if (sChangeStatus === sap.ui.comp.personalization.ChangeType.ModelChanged && this._bIsChartBound) {
