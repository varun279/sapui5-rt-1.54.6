...
sap.ui.define([
	...
	var DateRangeType = Type.extend("sap.ui.comp.config.condition.DateRangeType", /** @lends "sap.ui.comp.config.condition.DateRangeType.prototype */ {
		constructor: function(sFieldName, oFilterProvider, oFieldViewMetadata) {
			...
			if (oFieldViewMetadata && oFieldViewMetadata.precision) {
				this._Precision = (typeof oFieldViewMetadata.precision === "string" ? parseInt(oFieldViewMetadata.precision, 10) : oFieldViewMetadata.precision) || 0;
			}
	 ...
	 * @param {int} iPrecision fractions of a second (between 0...3) - number of decimal places. iPrecision<=0 will set 000, iPrecision==3 will set the millis to 999
	 * @returns {UniversalDate} the given date with the end time 23:59:59.999 UTC
	 */
	DateRangeType.setEndTime = function(oDate, bIgnoreTime, iPrecision) {
		oDate = DateRangeType.toUniversalDate(oDate);
		if (!bIgnoreTime) {
			...
			oDate.setSeconds(59);
			var ms = 0;
			var n = Math.min(Math.max(iPrecision, 0), 3);
			for (var i = 1; i <= n; i++) {
				ms += 9 * (Math.pow(10, 3 - i)); 
			}
			oDate.setMilliseconds(ms);
			return oDate;
		} else { 
	...
	DateRangeType.prototype.getFilterRanges = function() {
		...
		if (oCondition.operation === "FROM") {
			...
			oCondition.value1 = DateRangeType.setEndTime(oCondition.value1, this._bIgnoreTime, this._Precision).oDate;
		} else {
	...
	DateRangeType.prototype._getDateFormatter = function(bStrict) {
		...
		if (JSON.stringify(oFormatSettings) !== this._sFormatSettings) {
			...
			this._oDateFormatter = sap.ui.core.format.DateFormat.getInstance(oFormatSettings);
		}
