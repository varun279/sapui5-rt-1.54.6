...
sap.ui.define([
...
], function (ResourceModel, XMLComposite, commonHelper) {
	"use strict";
	...
	Field._helper = {
		formatDraftOwner: function (vDraftUUID, vDraftInProcessByUser, vDraftInProcessByUserDesc, vDraftLastChangedByUser, vDraftLastChangedByUserDesc, bHasDraftEntity) {
			...
			if (vDraftUUID && bHasDraftEntity) {
				...
				if (sUserDescription) {
					sDraftOwnerDescription = ResourceModel.getText("draft.OWNER", [sUserDescription]);
				} else {
					sDraftOwnerDescription = ResourceModel.getText("draft.ANOTHER_USER");
				}
			}
	...
	Field._helper.isSemanticKey.requiresIContext = true;
