...
sap.ui.define([
...
], function (ResourceModel, Controller, Sorter, JSONModel) {
	"use strict";
	return Controller.extend("sap.ui.mdc.internal.table.viewsettings.ViewSettings", {
		...
		_saveViewSettingsState: function (aCondition) {
			...
			// });
			// oViewSettingsModel.selectAllText = ResourceModel.getText("table.VIEWSETTINGS_COLUMN_SELECTALL", [aSelectColumnKeys.length, oViewSettingsModel.columnPanelItems.length]);
		...
		_updateSelectAllText: function () {
			var oColumnList = this.getView().byId("ColumnsList"),
				sSelectAllText = ResourceModel.getText("table.VIEWSETTINGS_COLUMN_SELECTALL", [oColumnList.getSelectedItems().length, oColumnList.getItems().length]);
