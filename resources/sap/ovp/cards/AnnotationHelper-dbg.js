...
(function () {
    ...
    sap.ovp.cards.AnnotationHelper.getLabelForDataItem = function (iContext, oDataItem) {
        if (!oDataItem) {
            return "";
        }
        if (oDataItem.Label) {
    ...
    **/
    ...
    function getDataFieldName(iContext, aCollection, index) {
        ...
        return sap.ovp.cards.AnnotationHelper.getLabelForDataItem(iContext, oDataField);
    }
    ...
    function getLabelForDataPoint(iContext, aCollection, index) {
        ...
        return sap.ovp.cards.AnnotationHelper.getLabelForDataItem(iContext, aSortedFields[0]);
    }
    ...
    function getTextArrangementBinding(iContext, oEntityType, oProperty, sIdBinding) {
        ...
        var oText = oProperty[sap.ovp.cards.AnnotationHelper.AnnotationType.TEXT];
        ...
        oTextArrangement = oProperty[sap.ovp.cards.AnnotationHelper.AnnotationType.TEXT_ARRANGEMENT];
        ...
        if (!sTextArrangementType) {
            oTextArrangement = oText && oText[sap.ovp.cards.AnnotationHelper.AnnotationType.TEXT_ARRANGEMENT];
            sTextArrangementType = oTextArrangement && oTextArrangement.EnumMember;
        }
        // 3. check TextArrangement definition for entity type - Priority 3 (min)
        if (!sTextArrangementType) {
            oTextArrangement = oEntityType && oEntityType[sap.ovp.cards.AnnotationHelper.AnnotationType.TEXT_ARRANGEMENT];
            sTextArrangementType = oTextArrangement && oTextArrangement.EnumMember;
        }
        ...
        var sDescriptionBinding = sap.ui.model.odata.AnnotationHelper.format(iContext, oText);
        ...
        switch (sTextArrangementType) {
            case sap.ovp.cards.AnnotationHelper.TextArrangementType.TEXT_FIRST :
                sFinalBinding = sDescriptionBinding + " (" + sIdBinding + ")";
                break;
            case sap.ovp.cards.AnnotationHelper.TextArrangementType.TEXT_LAST :
                sFinalBinding = sIdBinding + " (" + sDescriptionBinding + ")";
                break;
            case sap.ovp.cards.AnnotationHelper.TextArrangementType.TEXT_ONLY :
                sFinalBinding = sDescriptionBinding;
                break;
            case sap.ovp.cards.AnnotationHelper.TextArrangementType.TEXT_SEPARATE :
    ...
    function _formatField(iContext, oItem, oEntityType, oMetaModel, bDontIncludeUOM, bIncludeOnlyUOM, bUseSimplePath) {
        ...
        if (!bIncludeOnlyUOM) {
            ...
            //Text Arrangement to be applied only for table card
            if (sContentFragment && sContentFragment === "sap.ovp.cards.table.Table") {
    ...
    function formatDataPointState(iContext, aCollection, index) {
        var aDataPoints = getSortedDataPoints(iContext, aCollection);
        var oCriticalityConfigValues = sap.ovp.cards.AnnotationHelper.criticalityConstants.StateValues;
    ...
    sap.ovp.cards.AnnotationHelper.formatItems = function (iContext, oEntitySet) {
        ...
        var aFilters = sap.ovp.cards.AnnotationHelper.getFilters(oModel, oSelectionVariant);
    ...
    sap.ovp.cards.AnnotationHelper.getCardSelections = function (ovpCardProperties) {
        ...
        return {
            filters: sap.ovp.cards.AnnotationHelper.getFilters(ovpCardProperties, oSelectionVariant),
     ...
     */
    sap.ovp.cards.AnnotationHelper.getFilters = function (ovpCardProperties, oSelectionVariant) {
        var aFilters = [];
        var aSelectOptions;
    ...
    sap.ovp.cards.AnnotationHelper.formatDataFieldValueOnIndex = function (iContext, aCollection, index) {
        return formatDataField(iContext, aCollection, index);
    };

    sap.ovp.cards.AnnotationHelper.formatDataPointValueOnIndex = function (iContext, aCollection, index) {
     ...
     * @returns {*}
     */
    sap.ovp.cards.AnnotationHelper.formatDataPointOrField = function (iContext, aCollection, iDataPointIndex, iDataFieldIndex) {
        var iDataPointCount = sap.ovp.cards.AnnotationHelper.getDataPointsCount(iContext, aCollection);
        if (iDataPointCount > 0) {
            return sap.ovp.cards.AnnotationHelper.formatDataPointValueOnIndex(iContext, aCollection, iDataPointIndex);
        } else {
            return sap.ovp.cards.AnnotationHelper.formatDataFieldValueOnIndex(iContext, aCollection, iDataFieldIndex);
        }
    };
    ...
    sap.ovp.cards.AnnotationHelper.getLabelForFirstDataPoint = function (iContext, aCollection) {
        ...
        for (var i = 0; i < aCollection.length; i++) {
            ...
            if (oItem.RecordType === "com.sap.vocabularies.UI.v1.DataField") {
                ...
                if (oItem.IconUrl && oItem.IconUrl.Path) {
                    return sap.ui.model.odata.AnnotationHelper.simplePath(iContext, oItem.IconUrl);
                }
            }
        }
    };
     ...
     * @returns {string}
     */
    sap.ovp.cards.AnnotationHelper.formatUnit = function (iContext, aCollection, index) {
        var result = "";
        var oItem;
     ...
     * @returns {*}
     */
    sap.ovp.cards.AnnotationHelper.formatObjectNumber = function (iContext, aCollection, index, dontIncludeUOM) {
    ...
    sap.ovp.cards.AnnotationHelper.isFirstDataPointCriticalityForTableStatus = function (iContext, aCollection) {
    ...
    sap.ovp.cards.AnnotationHelper.formatDataPointValue = function (iContext, aCollection, dontIncludeUOM) {
        if (!aCollection) {
            return "";
        }
        var oModel = iContext.getSetting('ovpCardProperties');
    ...
    sap.ovp.cards.AnnotationHelper.checkCriticalityGenericForTableStatus = function (iContext, aCollection) {
        var oCopyOfCollection = jQuery.extend({}, aCollection);
        var oCopyOfContext = jQuery.extend({}, iContext);
        var oModel = iContext.getSetting('ovpCardProperties');
        var oEntityType = oModel.getProperty("/entityType");
    ...
    sap.ovp.cards.AnnotationHelper.resolveParameterizedEntitySet = function (oDataModel, oEntitySet, oSelectionVariant, aParameters) {
    ...
    */
    sap.ovp.cards.AnnotationHelper.semanticObjectOfFirstDataField = function (iContext, oEntitySet, aCollection) {
     ...
     */
    sap.ovp.cards.AnnotationHelper.semanticObjectOfSecondDataField = function (iContext, oEntitySet, aCollection) {
        if (!aCollection || !aCollection.length || aCollection.length === 0) {
            return false;
        }
    ...
    */
    sap.ovp.cards.AnnotationHelper.isFirstContactAnnotation = function (aCollection) {
        if (!aCollection || !aCollection.length || aCollection.length === 0) {
            return false;
        }
    ...
    sap.ovp.cards.AnnotationHelper.getAggregateNumber = function (iContext, oEntitySet, oDataPoint, oSelectionVariant) {
        ...
        var aFilters = sap.ovp.cards.AnnotationHelper.getFilters(oOvpCardSettings, oSelectionVariant);
     ...
     *
     */
    sap.ovp.cards.AnnotationHelper.TargetValueFormatter = function (iKpiValue, iTargetValue) {
        ...
        if (iValue) {
            var fnNumberFormat = sap.ui.core.format.NumberFormat.getFloatInstance({
    ...
    sap.ovp.cards.AnnotationHelper.formPathForTargetValue = function (oDataPointAnnotation) {
     ...
     *
     */
    sap.ovp.cards.AnnotationHelper.returnPercentageChange = function (iKpiValue, iTargetValue) {
        jQuery.sap.require("sap.ui.core.format.NumberFormat");
        ...
        if (iReferenceValue) {
            ...
            var fnPercentFormatter = sap.ui.core.format.NumberFormat.getPercentInstance({
                style: 'short',
                minFractionDigits: iFractionalDigits,
                maxFractionDigits: iFractionalDigits,
                showScale: true
    ...
    sap.ovp.cards.AnnotationHelper.formTheFilterByString = function (iContext, oSelectionVariant, aAllSelVar) {
        ...
        var aFilters = sap.ovp.cards.AnnotationHelper.getFilters(oCardPropsModel, oSelectionVariant);
        if (aFilters.length == 0) {
            return "";
        }
        var sProp;
    ...
    sap.ovp.cards.AnnotationHelper.formTheFilterByStringNotFromSelVar = function (iContext, oFilter, aFilters) {
        var bAppend = !(oFilter.index === (aFilters.length - 1));
        // build the filter string
        var sFilter = generateStringForFilters([aFilters[oFilter.index]]);
        if (bAppend === true) {
            return sFilter + ", ";
        }
        return sFilter;
    ...
    sap.ovp.cards.AnnotationHelper.formTheIdForFilterNotFromSelVar = function (oFilter) {
        return oFilter.id;
    };
    ...
    sap.ovp.cards.AnnotationHelper.formTheFilterByStringNotFromSelVar.requiresIContext = true;
    ...
    sap.ovp.cards.AnnotationHelper.formatDataPointOrField.requiresIContext = true;
    sap.ovp.cards.AnnotationHelper.formatDataFieldValueOnIndex.requiresIContext = true;
    ...
    sap.ovp.cards.AnnotationHelper.formatDataPointValueOnIndex.requiresIContext = true;
    ...
    sap.ovp.cards.AnnotationHelper.isFirstDataPointCriticalityForTableStatus.requiresIContext = true;
    ...
    sap.ovp.cards.AnnotationHelper.checkCriticalityGenericForTableStatus.requiresIContext = true;
    ...
    sap.ovp.cards.AnnotationHelper.semanticObjectOfFirstDataField.requiresIContext = true;
    sap.ovp.cards.AnnotationHelper.semanticObjectOfSecondDataField.requiresIContext = true;
    ...
    sap.ovp.cards.AnnotationHelper.getLabelForDataItem.requiresIContext = true;
    sap.ovp.cards.AnnotationHelper.getLabelForFirstDataPoint.requiresIContext = true;
    sap.ovp.cards.AnnotationHelper.formatObjectNumber.requiresIContext = true;
    sap.ovp.cards.AnnotationHelper.formatUnit.requiresIContext = true;
}());
