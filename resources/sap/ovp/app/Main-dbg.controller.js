...
sap.ui.define(["sap/ui/core/mvc/Controller", "sap/ui/model/odata/ODataUtils", "sap/ovp/cards/generic/Component", "sap/ui/generic/app/navigation/service/NavigationHandler",
        "sap/m/MessageBox", "sap/ovp/cards/CommonUtils", "sap/ovp/cards/PersonalizationUtils","sap/ovp/cards/charts/VizAnnotationManager", "sap/ushell/ui/footerbar/AddBookmarkButton", "sap/m/BusyDialog", "sap/ui/model/resource/ResourceModel",
    ...
    function (Controller, ODataUtils, Component, NavigationHandler, MessageBox, CommonUtils, PersonalizationUtils, VizAnnotationManager, AddBookmarkButton, BusyDialog, ResourceModel, Filter, SmartVariantManagement,
        ...
        return Controller.extend("sap.ovp.app.Main", {
            ...
            onInit: function () {
                ...
                this.oContainer = sap.ushell.Container;
            ...
            organizeCards: function (aOrderedCards) {
                ...
                for (var i = 0; i < aVisibleCards.length; i++) {
                    ...
                    if (aVisibleCards[i].template === "sap.ovp.cards.charts.smart.chart") {
                        /*Change the template to "sap.ovp.cards.charts.analytical" 
                         * if the chart type is donut or any time_series
                         * Or if the chart has a view switch
                         *  */
                        aVisibleCards[i].template = aVisibleCards[i].settings.tabs ? "sap.ovp.cards.charts.analytical" : this._getTemplateForChart(aVisibleCards[i]);
                    }
            ...
            _getTemplateForChart: function (oCard) {
                ...
                if (oCard && oCard.settings) {
                    ...
                    if (oCard.settings.kpiAnnotationPath) {
                        ...
                        if (sPresentationVariantPath) {
                            sTemplate = this._getTemplateForChartFromVisualization(sPresentationVariantPath, oCard, oModel, oEntitySet, oEntityType);
                        }
                    }
                    if (!sTemplate && oCard.settings.selectionPresentationAnnotationPath) {
                   ...
                   if (!sTemplate && oCard.settings.chartAnnotationPath) {
                       ...
                       if (chartTypeEnum && chartTypeEnum.split("/")[1] === "Donut") {
                           sTemplate = "sap.ovp.cards.charts.analytical";
                       } else {
                           var oConfig = sap.ovp.cards.charts.VizAnnotationManager.getConfig(oChartAnnotation.ChartType);
                           bSupportsTimeSemantics = sap.ovp.cards.charts.VizAnnotationManager.hasTimeSemantics(oChartAnnotation.DimensionAttributes, oConfig, oModel, oEntitySet.name);
                           if (bSupportsTimeSemantics) {
                               sTemplate = "sap.ovp.cards.charts.analytical";
                           } else {
                               sTemplate = "sap.ovp.cards.charts.smart.chart"; //For all the chart cards except donut and timeseries
                           }
                       }
                   }
                   return sTemplate;
            ...
            _getTemplateForChartFromVisualization: function(sPresentationVariantPath, oCard, oModel, oEntitySet, oEntityType) {
                ...
                var aVisualizations = oEntityType[sPresentationVariantPath].Visualizations;
                var index;
                var oChartAnnotation;
                var chartTypeEnum;
                var bSupportsTimeSemantics;
            ...
            onGlobalFilterSearch: function () {
                ...
                if (this.filterChanged || this.bGoButtonPressed) {
                    var oGlobalFilter = this.getGlobalFilter();
                    if (oGlobalFilter && !oGlobalFilter.isDialogOpen()) {
                        this._storeCurrentAppStateAndAdjustURL();
                    }
            ...
            * */
                ...
                if (oLayout) {
                    if (oChange) {
                        if (!Array.isArray(oChange)) {
                            this.oFlexController.createAndApplyChange(oChange, oLayout);
                        } else {
                            oChange.forEach(function(oOneChange) {
                                this.oFlexController.createAndApplyChange(oOneChange, oLayout);
                            }.bind(this));
                        }
                    ...
                    cells: [
                        ...
                        new Switch({
                            ...
                            change: function (event) {
                                ...
                                var iIsCardChangedAgain = this.visibilityChanges.findIndex(function (oChange, iIndex, arr) {
                                    return oChange.content.cardId === oCard.id;
                                });
                        ...
                        sap.m.MessageBox.show(this._getLibraryResourceBundle().getText("reset_cards_confirmation_msg"), {
                            ...
                            onClose: function (oAction) {
                                if (oAction === sap.m.MessageBox.Action.OK) {
                                    ...
                                    // TODO: handle promise reject case for discarding changes
                                    this.oFlexController.discardChangesForId(this.getLayout().getId(), true).then(function (arg) {
            ...
            _getCustomAppState: function () {
                ...
                var oCustomDataRet = this.getCustomAppStateDataExtension(oCustomData);
                if (oCustomDataRet) {
                    oCustomData = oCustomDataRet;
                }
                return oCustomData;
            },
